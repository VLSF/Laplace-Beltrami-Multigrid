...
class equation:
    ...
    def defects(self, current):
        ...
        return mu.defects(current, COEFF, RHS)
    ...
    def residuals(self, current):
        ...
        COEFF = self.coefficients(current)
        RHS = self.rhs(current)
        return mu.residuals(current, COEFF, RHS)
    ...
    def errors(self, current):
        ...
        return mu.errors(current, EXACT)
    ...
    def operator(self, current):
        ...
        current = self.bc(current)
        COEFF = self.coefficients(current)
        return mu.operator(current, COEFF)
...
class harmonic_equation(equation):
    ...
    def produce_initial_guess(self, J):
        N = 2 ** J + 1
        z = np.linspace(0, 1, N)
        current = np.array(np.meshgrid(z, z, indexing='ij'))
        current = self.bc(current)
        return current
...
def get_harmonic(name):
    ...
    if name == 'Frozen Metrics':
        res = harmonic_equation(basic_harmonic_coeff, trivial_harmonic_rhs, 2, bc=trivial_harmonic_bc)
    if name == 'Harmonic with Frozen Metrics':
        res = harmonic_equation(harmonic_coeff, trivial_harmonic_rhs, 2, bc=trivial_harmonic_bc)
    if name == 'Upwind + Downwind Frozen Metrics':
